...
- **라 rally & 대형**: 최초 적을 찾으면 랠리 포인트(목표에서 `Constants.RALLY_DISTANCE` 뒤쪽)를 잡고 리더가 이동, 나머지는 회전된 오프셋으로 대형 유지. 랠리 도착(`Constants.FORMATION_THRESHOLD`) 시 교전 모드 전환.
