...
public static partial class XmlConverter
{
    ...
    /// <summary>
    /// SheetData를 bytes 파일로 저장합니다 (XML을 UTF-8 바이트로 저장).
    /// </summary>
    /// <param name="sheetData">저장할 시트 데이터</param>
    /// <param name="outputDirectory">출력 디렉토리</param>
    /// <returns>저장된 파일 경로</returns>
    public static string SaveToBytesFile(SheetData sheetData, string outputDirectory)
    {
        ArgumentNullException.ThrowIfNull(sheetData);
        ...
        var safeFileName = GetSafeFileName(sheetData.SheetName);
        var filePath = Path.Combine(outputDirectory, $"{safeFileName}.bytes");

        var xmlContent = ConvertToXml(sheetData);
        File.WriteAllBytes(filePath, Encoding.UTF8.GetBytes(xmlContent));

        return filePath;
    }

    /// <summary>
    /// SheetData를 JSON 파일로 저장합니다.
    /// </summary>
    /// <param name="sheetData">저장할 시트 데이터</param>
    /// <param name="outputDirectory">출력 디렉토리</param>
    /// <returns>저장된 파일 경로</returns>
    public static string SaveToJsonFile(SheetData sheetData, string outputDirectory)
    {
        ArgumentNullException.ThrowIfNull(sheetData);
        ...
        var safeFileName = GetSafeFileName(sheetData.SheetName);
        var filePath = Path.Combine(outputDirectory, $"{safeFileName}.json");

        var jsonContent = ConvertToJson(sheetData);
    ...
    /// <summary>
    /// SheetData를 JSON 문자열로 변환합니다.
    /// </summary>
    public static string ConvertToJson(SheetData sheetData)
    {
        ArgumentNullException.ThrowIfNull(sheetData);
        ...
        return JsonSerializer.Serialize(new { SheetName = sheetData.SheetName, Data = rows }, new JsonSerializerOptions { WriteIndented = true });
    }
    ...
    /// <summary>
    /// 여러 시트 데이터를 각각 bytes 파일로 저장합니다.
    /// </summary>
    /// <param name="sheetsData">저장할 시트 데이터 목록</param>
    /// <param name="outputDirectory">출력 디렉토리</param>
    /// <returns>저장된 파일 경로 목록</returns>
    public static IList<string> SaveAllToBytesFiles(IList<SheetData> sheetsData, string outputDirectory)
    {
        ArgumentNullException.ThrowIfNull(sheetsData);
        ...
        foreach (var sheetData in sheetsData)
        {
            var path = SaveToBytesFile(sheetData, outputDirectory);
            savedPaths.Add(path);
        }
    ...
    /// <summary>
    /// 여러 시트 데이터를 각각 JSON 파일로 저장합니다.
    /// </summary>
    /// <param name="sheetsData">저장할 시트 데이터 목록</param>
    /// <param name="outputDirectory">출력 디렉토리</param>
    /// <returns>저장된 파일 경로 목록</returns>
    public static IList<string> SaveAllToJsonFiles(IList<SheetData> sheetsData, string outputDirectory)
    {
        ArgumentNullException.ThrowIfNull(sheetsData);
        ...
        foreach (var sheetData in sheetsData)
        {
            var path = SaveToJsonFile(sheetData, outputDirectory);
            savedPaths.Add(path);
        }
